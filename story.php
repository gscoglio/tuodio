...
if(isset($requestTitle)){$requestID = $db->get_var($sql="SELECT link_id FROM " . table_links . " WHERE `link_title_url` = '".sanitize($requestTitle,4)."';");}
...
//		$main_smarty->display($the_template . '/pligg.tpl');		
	...
	$main_smarty->assign('URL_rss_page', getmyurl('storyrss', isset($requestTitle) ? $requestTitle : $requestID));
...
//	$comments = $db->get_col("SELECT comment_id FROM " . table_comments . " WHERE comment_link_id=$link->id and comment_parent = 0 ORDER BY " . $CommentOrderBy);
	$comments = $db->get_results("SELECT * FROM " . table_comments . " WHERE comment_status='published' AND comment_link_id=$link->id and comment_parent = 0 ORDER BY " . $CommentOrderBy);
	if ($comments) {
		...
		foreach($comments as $dbcomment) {
			...
			$comments2 = $db->get_results("SELECT * FROM " . table_comments . " WHERE comment_status='published' AND comment_parent={$dbcomment->comment_id} ORDER BY " . $CommentOrderBy);
			if ($comments2) {
