...
switch ($sortby) {
	...
	case 3: //sort users by number of comments
		$select = "SELECT user_id, count(*) as count ";
		//$from_where = " FROM " . table_comments . ", " . table_users . " WHERE comment_status='published' AND comment_user_id=user_id AND user_level<>'god' GROUP BY comment_user_id";
		$from_where = " FROM " . table_comments . ", " . table_users . " WHERE comment_status='published' AND comment_user_id=user_id GROUP BY comment_user_id";
		$order_by = " ORDER BY count DESC ";
		break;
