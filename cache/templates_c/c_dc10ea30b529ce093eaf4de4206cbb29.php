<?php /* V2.10 Template Lite 4 January 2007  (c) 2005-2007 Mark Dickenson. All rights reserved. Released LGPL. 2011-07-17 15:15:36 PDT */  echo tpl_function_checkActionsTpl(array('location' => "tpl_admin_modules_top"), $this);?> 
<?php 
			...
			foreach($modules as $module) {
				...
				if(isset($module_info['settings_url'])){
					$settings_url = $module_info['settings_url'];
						echo '<td><a href="' . $settings_url . '">Settings</a></td>';
					} else {
						echo '<td>No Settings</td>';
					}
				
				if(isset($module_info['homepage_url'])){
					$homepage_url = $module_info['homepage_url'];
						echo '<td><a href="' . $homepage_url . '" target=_blank">Homepage</a></td>';
					} else {
						echo '<td>No Homepage</td>';
					}
				}
		...
		foreach($foundfolders as $key => $value){
			...
			if($module_info = include_module_settings($value)){
				...
				if(file_exists('../modules/' . $value . '/' . $value . '_readme.htm')){
					...
					echo '<table class="modules"><tr><td width="10%"><a href = "?action=install&module=' . $value . '">Install</a></td><td width="30%"><p>' . $thename . '</p></td>';
				}
