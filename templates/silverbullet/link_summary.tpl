<div class="news-summary" id="xnews-{$link_shakebox_index}"{if $link_shakebox_currentuser_reports gt 0}style="opacity:0.3;filter:alpha(opacity = 30)"{/if}>
...
<div class="news-vote">
		...
		<div class="vote-count"><span id="xvotes-{$link_shakebox_index}">{$link_shakebox_votes} </span><span class="voter">vote</span></div>
		{/if}
		
		{if $link_shakebox_votes  gt 1}	
		<div class="vote-count"><span id="xvotes-{$link_shakebox_index}">{$link_shakebox_votes} </span><span class="voter">votes</span></div>
		{/if}
		
		<ul>

		<li class="vote-r" id="xvote-{$link_shakebox_index}">
	...
	<div class="top"{if $Voting_Method eq 2} style="margin-left:20px;padding:0"{/if}> 
...
<div class="l_border">
		<div class="news-submitted">
			{if $use_thumbnails eq true && $pagename eq "story" && $url_short neq "http://" && $url_short neq "://"}
				<img src="http://images.websnapr.com/?url={$url_short}" width="90px" height="70px" alt="snap"/>
			{/if}
			...
			<span id="ls_adminlinks-{$link_shakebox_index}" style="display:none">
				...
				{if $Voting_Method eq 2}<div style="line-height: 28px;margin-top:5px;">
			<ul class='star-rating{$star_class}' id="xvotes-{$link_shakebox_index}" style="margin:0">
		 ...
		 - <strong>{#PLIGG_Visual_Vote_Average#}: <span id="mnma-{$link_shakebox_index}">{$link_rating}</span> {#PLIGG_Visual_Vote_Average_With#} <span id="mnmb-{$link_shakebox_index}">{$vote_count}</span> {#PLIGG_Visual_Vote_Average_Votes#}</strong>
	    ...
	    <div class="news-details">
	    	<span id="ls_comments_url-{$link_shakebox_index}">
	    		{if $story_comment_count eq 0}
	    			...
	    			<span class="linksummaryDiscuss"><a href="{$story_url}" class="comments">{#PLIGG_MiscWords_Discuss#}</a></span>
	    		{/if}
	    		{if $story_comment_count eq 1}
	    			<span class="linksummaryHasComment"><a href="{$story_url}" class="comments2">{$story_comment_count} {#PLIGG_MiscWords_Comment#}</a></span>
	    		{/if}
	    		{if $story_comment_count gt 1}
	    			<span class="linksummaryHasComment"><a href="{$story_url}" class="comments2">{$story_comment_count} {#PLIGG_MiscWords_Comments#}</a></span>
	    		{/if}
	    	</span>  
			...
			{if $link_category neq ""}
				<span id="ls_category-{$link_shakebox_index}">{#PLIGG_MiscWords_Category#}: <a href="{$category_url}">{$link_category}</a></span>
			{else}
			
			{/if}

			{if $enable_tags eq 'true' && $pagename eq "story"}
				{if $tags ne ''}<br/><br/>
					<span id="ls_tags-{$link_shakebox_index}">{#PLIGG_Visual_Tags_Link_Summary#}: 
			...
			{if $Enable_AddTo eq 1}
				<span id="addto-{$link_shakebox_index}" style="display:none"><br />{#PLIGG_Visual_LS_AddTo#}
					&nbsp;&nbsp;<a title="submit '{$title_short}' to del.icio.us" href="http://del.icio.us/post" onclick="window.open('http://del.icio.us/post?v=4&amp;noui&amp;jump=close&amp;url={$enc_url}&amp;title={$enc_title_short}', '{#PLIGG_Visual_LS_Delicious#}','toolbar=no,width=700,height=400'); return false;"><img src="{$my_base_url}{$my_pligg_base}/templates/{$the_template}/images/delicious.png" border="0" alt="submit '{$title_short}' to del.icio.us" height="16px" width="16px"/></a>
					&nbsp;&nbsp;<a title="submit '{$title_short}' to digg" href="http://digg.com/submit?phase=2&amp;url={$enc_url}&amp;title={$title_short}"><img src="{$my_base_url}{$my_pligg_base}/templates/{$the_template}/images/digg.png" border="0" alt="submit '{$title_short}' to digg" height="16px" width="16px"/></a>
					&nbsp;&nbsp;<a title="submit '{$title_short}' to reddit" href="http://reddit.com/submit?url={$enc_url}&amp;title={$title_short}"><img src="{$my_base_url}{$my_pligg_base}/templates/{$the_template}/images/reddit.gif" border="0" alt="submit '{$title_short}' to reddit" height="16px" width="16px"/></a>
					&nbsp;&nbsp;<a title="submit '{$title_short}' to Pligg" href="http://pligg.com/submit.php?url={$enc_url}&amp;title={$title_short}"><img src="{$my_base_url}{$my_pligg_base}/templates/{$the_template}/images/pligg.png" border="0" alt="submit '{$title_short}' to Pligg" height="16px" width="16px"/></a>
					&nbsp;&nbsp;<a title="submit '{$title_short}' to yahoo" href="http://myweb2.search.yahoo.com/myresults/bookmarklet?u={$enc_url}&amp;title={$title_short}"><img src="{$my_base_url}{$my_pligg_base}/templates/{$the_template}/images/yahoomyweb.png" border="0" alt="submit '{$title_short}' to yahoo" height="16px" width="16px"/></a>
