...
//###STOP FORUM SPAM PATCH###
	if(register_check_spammer(trim($username),trim($userip),trim($email))){
			$form_username_error[] = $main_smarty->get_config_vars('PLIGG_Visual_Register_Error_UserInvalid');
			//we DO NOT tell them that their IP address is banned!
			$error = true;
	}
//###STOP FORUM SPAM PATCH###
...
function register_check_spammer($username,$userip,$email){
	...
	if ($content !== false) {
	  $isSpammer = strpos($content,"yes")?true:$isSpammer;
		} else {
		 //do nothing?
		}
		...
		$logstr = "Last Blocked ".$username.", ".$userip.", ".$email.", ".date("F j - Y - g:i a")."\r\n";
		file_put_contents('registerLog.txt',$logstr, FILE_APPEND);
		return $isSpammer;
}
//###STOP FORUM SPAM PATCH###
