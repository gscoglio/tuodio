...
function member_display($requestID)
{
	...
	if($member)
	{
		foreach($member as $memberid)
		{
			...
			$member_display .= '<img src="'.$group_member_avatar.'" /><a class="group_member" href="'.$group_member_url.'"> '.$member_name.'</a>';
			if($gcreator == $current_user->user_id)
			{
...
function group_stories($requestID,$catId)
{
	...
	if ($catId) 
		$from_where = " AND (link_category=$catId)";
	...
	if ($_GET['view'] == 'upcoming')
		...
		$from_where = " AND (link_votes>=$group_vote OR link_status='published')";

	$offset = (get_current_page()-1)*$page_size;
	$links = $db->get_results($sql = "SELECT SQL_CALC_FOUND_ROWS * FROM " . table_links . " WHERE link_group_id = $requestID AND link_group_status!='discard' $from_where LIMIT $offset, $page_size");
...
function group_shared($requestID,$catId)
{
	...
	$offset = (get_current_page()-1)*$page_size;
	$links = $db->get_results($sql="SELECT SQL_CALC_FOUND_ROWS b.* FROM " . table_group_shared . " a
					    LEFT JOIN " . table_links . " b ON link_id=share_link_id
					    WHERE share_group_id = $requestID AND !ISNULL(link_id) $from_where LIMIT $offset, $page_size");
   	$rows = $db->get_var("SELECT FOUND_ROWS()");
	if ($links) {
		foreach($links as $dblink) {
			$link->id=$dblink->link_id;
